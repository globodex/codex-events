...
<!-- SECTION:NOTES:BEGIN -->
Implemented explicit operation domains across every MCP-eligible structured route and removed per-operation toolName metadata. Macro grouping now uses only the operation domain and existing effect. The route audit confirmed 169 one-line metadata replacements, with only the five approved prize operations changing macro domain. Exact persona snapshots verify unchanged permissions and the approved prize move. Integration coverage verifies successful and failed mutation audits from the selected operation, one actor resolution per request, and direct unauthorized rejection. Validation passed: bun run lint; bun run typecheck; bun run test:unit with 172 files and 1,143 tests; bun run test:integration with 44 files and 496 tests; bun run test:bdd with 92 regular and 2 destructive scenarios; current-head GitHub CI including CodeQL and both backend and topology runs; and git diff --check. No automation gaps, setup changes, task-specific risks, or follow-up work remain.
<!-- SECTION:NOTES:END -->
