...
When('I add a code giveaway in the simplified event builder', async ({ page }) => {
  ...
  await expect(manager.getByTestId('giveaway-instructions').getByRole('button', { name: 'bold', exact: true })).toBeVisible()
  await manager.getByTestId('giveaway-instructions').getByRole('textbox').fill('Use **this code** in *billing*.\n\n- [Open billing](https://example.com/billing)')
...
Then('I should see automatically detected codes and the combined email preview', async ({ page }) => {
  ...
  await expect(page.getByTestId('event-builder-settings-luma-sync')).toBeVisible()
  ...
  await expect(dialog.locator('strong')).toContainText('this code')
  await expect(dialog.locator('em')).toContainText('billing')
  await expect(dialog.getByRole('listitem')).toContainText('Open billing')
  await expect(dialog.getByRole('link', { name: 'Open billing' })).toHaveAttribute('href', 'https://example.com/billing')
...
When('I open the connected Luma event in the builder', async ({ page }) => {
  await applyStoredStateToPage('event_admin', page)
  await page.goto(`/admin/events/builder/${fixtureSlug}`)
  await waitForNuxtHydration(page)
})

Then('I can edit and cancel the saved Luma connection without losing it', async ({ page }) => {
  const luma = page.getByTestId('event-builder-settings-luma-sync')
  await expect(luma.getByText('Connected', { exact: true })).toBeVisible()
  await expect(luma.getByText('evt-bdd-simplified', { exact: true })).toBeVisible()
  await expect(luma.getByLabel('Luma API key', { exact: true })).toBeHidden()
  await expect(luma.getByRole('button', { name: 'Import existing check-ins' })).toBeEnabled()
  await luma.getByRole('button', { name: 'Change connection' }).click()
  await expect(luma.getByLabel('Luma event ID', { exact: true })).toHaveValue('evt-bdd-simplified')
  await expect(luma.getByLabel('Luma API key', { exact: true })).toHaveValue('bdd-luma-key')
  await luma.getByLabel('Luma event ID', { exact: true }).fill('evt-unsaved')
  await expect(luma.getByRole('button', { name: 'Import existing check-ins' })).toBeHidden()
